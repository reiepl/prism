...
interface AddPersonModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (person: Omit<Person, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>;
}
...
export const AddPersonModal: React.FC<AddPersonModalProps> = ({
  ...
  const handlePhotoUpload = (file: File) => {
    ...
    // Limit to 2MB to keep IndexedDB happy
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center p-0 sm:p-4">
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 shrink-0">
              <div>
                <h2 className="text-xl font-display font-bold text-slate-800">Add Person</h2>
                <p className="text-xs text-slate-400 font-medium">Add a new connection to your network</p>
              </div>
              <button
                id="close-add-modal"
                onClick={() => {
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="person-nickname" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                      Nickname
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label htmlFor="person-relationship" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                      Relationship
                    </label>
                    <select
                ...
                {/* Date of Birth */}
                <div>
                  <label htmlFor="person-dob" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                ...
                {timeAccuracy === 'ShiChen' && (
                  ...
                  >
                    <label htmlFor="birth-time-shichen" className="block text-xs font-bold text-slate-700 uppercase tracking-wider">
                      Birth Time (Shi Chen)
                    </label>
                    <select
                      id="birth-time-shichen"
                      value={birthTime}
                      onChange={(e) => setBirthTime(e.target.value)}
                      className="w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm text-slate-800 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all"
                    >
              ...
              <div className="flex gap-3 pt-4 shrink-0 border-t border-slate-100">
                <button
                  ...
                  className="flex-1 py-3 px-4 border border-slate-200 text-slate-600 font-semibold text-sm rounded-xl hover:bg-slate-50 active:bg-slate-100 transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-3 px-4 bg-slate-900 hover:bg-slate-800 active:bg-black text-white font-semibold text-sm rounded-xl transition-colors cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {isSubmitting ? (
